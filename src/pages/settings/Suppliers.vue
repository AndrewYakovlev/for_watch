<template>
  <div class="setting_page">
    <div class="page_header">
      ...
      <el-button type="primary" @click="create" icon="el-icon-plus">Поставщик</el-button>
    </div>
...
import supplierEntity from "@/objects/entities/supplier.entity";

export default {
  name: "Suppliers",
  computed: {
    ...mapState({
      rows: state => state.suppliers.rows,
      loading: state => state.suppliers.loading
    })
  },
  methods: {
    create() {
      this.$store.dispatch(ui.supplierDrawer, {status: true, element: supplierEntity.empty})
    },
    edit(element) {
      this.$store.dispatch(ui.supplierDrawer, {status: true, element})
    }
  }
}
</script>
