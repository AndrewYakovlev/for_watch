<template>
  <div class="setting_page">
    <div class="page_header">
      <h1 class="title">Кассы</h1>
      <el-button type="primary" @click="create" icon="el-icon-plus">Касса</el-button>
    </div>
...
import cashboxEntity from "@/objects/entities/cashbox.entity";
...
export default {
  ...
  computed: {
    ...mapState({
      rows: state => state.cashboxes.rows,
      loading: state => state.cashboxes.loading
    })
  },
  methods: {
    create(){
      this.$store.dispatch(ui.cashboxDrawer, {status: true, element: cashboxEntity.empty})
    },
    edit(element){
      this.$store.dispatch(ui.cashboxDrawer, {status: true, element})
    }
  }
}
</script>
